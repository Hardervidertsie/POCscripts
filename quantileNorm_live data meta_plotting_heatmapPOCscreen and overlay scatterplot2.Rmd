...
#pdf(file = paste(zeh.path,"/model fit graphs/model fit graphs_raw.pdf", sep =""), height = 6, width = 6)
...
#    ggtitle(unique(my.data.response.list[[i]]$splitL)) 
...
# only nrf2  repl1 is lacking, etoposide dose.f 3 is missing. input as NA for equal length
order(c(1,2,NA,3))

dummyEtoposide <- model.results.df[ cell_line == "Nrf2" & treatment == "Etoposide" & replID == "repl.2" & dose.f == 3]
dummyEtoposide[ , replID := "repl.1"]
dummyEtoposide[, mod := sample(model.results.df[ cell_line == "Nrf2" & replID == "repl.1", mod ], 23)] # need fake numbers, remove after order/ sort operations
dummyEtoposide[, resid.SD := NA]

model.results.df <- rbind(model.results.df, dummyEtoposide)
...
quantileOrigList <- split(model.results.df, model.results.df$splitL)
 # sort per plate
...
model.results.df <- model.results.df[ !(cell_line == "Nrf2" & treatment == "Etoposide" & replID == "repl.1" & dose.f == 3)] # remove the dummy dated that was needed for sort/ quantile normalization

model.results.df.sd <- model.results.df[ , sd( quantileNorm ), by = list( cell_line, treatment , dose.f ,timeAfterExposure)]
...
DMSO.s <- DMSO[ ,  list( DMSO.mean = mean( quantileNorm ), 
                         DMSO.sd = sd( quantileNorm ),
                         DMSO.resid = sqrt( mean(resid.SD^2)) ),  
...
setkey(model.results.df.stats, cell_line, i.treatment, dose.f, timeAfterExposure )
...
model.results.df.stats[ , meanDiff :=  (quantileNorm - DMSO.mean)  / sqrt ( ( DMSO.sd^2 + DMSO.resid^2 + treat.sd^2 + resid.SD^2 ))  ]  # adding variances for total sd



Stats.m <- model.results.df.stats[, mean(meanDiff), by = list( cell_line, treatment, dose_uM, dose.f, replID  )]

Stats.m[, cell.treat.dose := paste( cell_line, treatment, dose.f, sep = "_")]


as.data.frame (Stats.m[ cell_line == "Nrf2" & treatment == "DMSO"])

setnames(Stats.m, "V1", "meanDiff")

Stats.m[ treatment %in% "DMSO" , cell.treat.dose := gsub("_[0-4]{1}", "", cell.treat.dose) ]
...
for( i in seq_along( all.vars )) {
  ...
  cur.treat <-unique( Stats.m[  cell.treat.dose %in% all.vars[ i ], treatment ] )
  subSet1 <- Stats.m[ cell.treat.dose %in% all.vars[ i ] ]
  subSetDMSO <- Stats.m[ treatment %in% "DMSO" & cell_line %in% cur.cell_line ]
  subSetDMSO <- subSetDMSO[ ,mean( meanDiff ), by = list( cell_line ,treatment,  dose_uM, dose.f, cell.treat.dose )] 
  ...
  stats.out[[i]] <- t.test(meanDiff ~ treatment,  var.equal = TRUE, data =   subSet, alternative = "less" )
...
write.table(file = paste(save.path, "stats.out.data.df_3sdSUM_quantile_au.txt", sep =""), stats.out.data.df, sep ="\t", row.names = FALSE)
...
pdf(file = paste( save.path, "model fit graphs means/model fit graphs means_quantile.pdf", sep =""), height = 6, width = 6) 
...
pdf(paste(save.path, "timecourses_1sided_sd3SUM_quantile_AU.pdf", sep =""), height = 14, width = 22)
p<-ggplot( model.results.df.m, aes(x=timeAfterExposure, y = meanQ, color = dose.f)) +
...
save.image(paste(save.path, "freeze20052016.Rdata", sep = ""))

load(paste(save.path, "freeze20052016.Rdata", sep = ""))
...
for(i in  seq_along(cells)) {
  ...
  time.vectors.perCell <-  as.data.frame( 
    dcast(
      data = buffer.cells, treat.dose~timeAfterExposure, value.var = "meanQ" )
  )
  ...
  method = "euclidean", diag = TRUE ) #c("euclidean", "maximum", "manhattan", "canberra", "binary" , "minkowski")[1],diag=TRUE) #, p=2
...
for(i in  seq_along(comp.doses)) {
  ...
  dist.cells[[i]] <- dist(time.vectors.perCompound, method = 
                            c("euclidean", "maximum", "manhattan", "canberra", "binary" , "minkowski")[1],  diag=TRUE)

}

dist.cells <- lapply(dist.cells, function(x) x<- as.matrix(x))
my.array<- array(unlist(dist.cells), c(11,11,27))
my.array[1,1,27]
...
results.treat.dose.order
  treatmentAnot[!treatmentAnot$treatment %in%results.treat.dose.order ,]

reord.rownames <- results.treat.dose.order

results.cell.order
[1] "p53BP1" "p21"    "Btg2"   "p53"    "CHOP"   "Xbp1"   "Atf4"   "BiP"    "Nrf2"   "Keap1"  "Srxn1"  
>    
results.treat.dose.order

# now for each cell:
dir()
dir.create( paste( save.path , "timeHM", sep = ""))

pic.place <- paste( save.path , "timeHM/", sep = "")
...
my.seq<-seq(min(all.norm.data$meanQ),max(all.norm.data$meanQ), length.out=10)
...
pdf(file = paste(pic.place, paste("live cubic distance HM_", cells[i], ".pdf", sep =""), sep=""), width = 5, height = 14)
...
save.image(file = paste( save.path, "freeze23052016.Rdata", sep =""))
load(paste( save.path, "/freeze23052016.Rdata", sep =""))
#reviewer asked for clustering at distinct time points: e.g. 6hr 12hr and 18hr
head( model.results.df.m )


# first heatmap of 24 hour point
getwd()
treatment_type <- c("light blue", "darkred", "green", "orange")
names(treatment_type) <-  c( "ox_stress","DDR","ER_stress", "control"  )
#write.table(treatmentAnot, file = "treatmentAnot2.txt", col.names=T, sep ="\t", row.names = F)
...
colnames(reporterAnot)  <- "reporter_type"
myColors <- list(reporter_type = reporter_type, treatment_type = treatment_type)
all.norm.data<- as.data.table(all.norm.data)
unique(all.norm.data[ ,timeAfterExposure ])

data.24h <- (all.norm.data[ timeAfterExposure ==  23.88 ])

my.data.w <-  dcast( data = data.24h, treat.dose~cell_line, value.var = "meanQ" )

my.data.w <- as.data.frame(my.data.w)
rownames(my.data.w) <- my.data.w$treat.dose
my.data.w$treat.dose<- NULL


 d <- hclust(dist( t(as.matrix(my.data.w)), method = "manhattan" ), method = "complete")
 
 pdf( paste(save.path, "single tp hm/", "18h dendogram cells.pdf" , sep = "") , height = 4, width = 6)
 plot(d)
 dev.off()

   cp <- hclust(dist( (as.matrix(my.data.w)), method = "manhattan" ), method = "complete")
 pdf( paste(save.path, "single tp hm/", "18h dendogram treatment.pdf" , sep = "") , height = 6, width = 10)
 plot(cp)
 dev.off()
 ...
 reporterAnot<- data.frame(reporter_type = c("DDR", "DDR","DDR", "DDR",  "ox_stress", "ox_stress",  "ox_stress","ER_stress", "ER_stress", "ER_stress","ER_stress"))
 
 
 
 w <- c(8,  9,         2,       10,       6,      5,      7,        11,     1,       3,       4)
 "Atf4"   "BiP"    "Btg2"   "CHOP"   "Keap1"  "Nrf2"   "Srxn1"  "Xbp1"   "p21"    "p53"    "p53BP1"
...
dr <-reorder( x = as.dendrogram( d ) ,wts = w , agglo.FUN = mean )
par(mfrow = c(3,1))
plot(d) 
plot(dr)
...
orderCols <- c("Atf4"   ,"BiP"  ,  "Btg2"  ,"CHOP"  , "Keap1" , "Nrf2" ,  "Srxn1" , "Xbp1" ,  "p21" ,   "p53" ,   "p53BP1")[d$order]

orderCols <- orderCols[ c(1,2,3,5,4,6,7,8,9,10,11)]

head(my.data.w)

indcol <- match(orderCols, colnames( my.data.w))
orderCols
 ...
 # what spot is each label in the ordered dendogram?
 
 wc <- match(cp$labels, cp$labels[ cp$order ])
 # use these as weights, should not change the ordering, CDDO 0.008 from 17 -> 19, and 19 --> 17
 wc[4] <- 19 
 wc[14] <- 17
 dcr <-reorder( x = as.dendrogram( cp ) ,wts = wc , agglo.FUN = mean )
 plot(dcr)
 ...
 indc <- match( labels(dcr),rownames(my.data.w))
 rownames( my.data.w)[indc]
 
 
 
 my.data.w <- my.data.w[ indc, indcol ]
 
 rownames(my.data.w)
 colnames(my.data.w)
 par(mfrow = c(1,1))
 #distfun = "manhattan", hclustfun ="complete",
 ?aheatmap
 treatmentAnot
 
 plot(dr)
 indd <- match( rownames(my.data.w), treatmentAnot$treatment)
 
 treatmentAnot$treatment[indd]
 treatmentAnot<- treatmentAnot[ indd, ]
 treatmentAnot$treatment <- NULL
...
pdf(file = paste( save.path, "dendo_24hrscompounds.pdf", sep = ""), width = 10, height =6 )
print(plot( dcr ))
dev.off()
save.path <- "C:/Users/steve_000/Documents/work/POC paper/"
save.image(paste(save.path, "freeze29052016.Rdata", sep =""))

load( paste( save.path, "freeze29052016.Rdata", sep =""))
...
# ff zijpaneel plotjes van POC fig 5A
all.norm.data[ treat.dose %in% "DMSO 75"]
as.data.frame(all.norm.data[ cell_line == "Atf4"])
unique(all.norm.data$cell_line)
subSetje <- model.results.df.m[ treat.dose %in% c("DMSO 75", "Etoposide 25" ) & cell_line %in% c( "p53BP1", "p53", "p21", "Btg2")  ]
subSetje[ , treat.dose := factor( treat.dose, levels = c("DMSO 75", "Etoposide 25" ))]
subSetje[ , cell_line := factor( cell_line, levels = c( "p53BP1", "p53", "p21", "Btg2") )]
ggplot(data = subSetje,
       aes( x = timeAfterExposure, y = meanQ )) + theme_bw() + geom_point(size = 3) + geom_line() + facet_grid(cell_line ~ treat.dose ) 
